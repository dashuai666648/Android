...
public class NoteEditor extends Activity {
    ...
    private static final String[] PROJECTION =
        new String[] {
            ...
            NotePad.Notes.COLUMN_NAME_TODO_COMPLETED
    };

    // Activity保存状态的标签
    private static final String ORIGINAL_CONTENT = "origContent";

    // 此Activity可以通过多个操作启动。每个操作表示为
    // 一个"状态"常量
    ...
    private CheckBox mIsTodoCheckBox;
    private CheckBox mTodoCompletedCheckBox;

    /**
     * 定义一个自定义EditText View，在显示的每行文本之间绘制线条。
     */
    public static class LinedEditText extends EditText {
        ...
        public LinedEditText(Context context, AttributeSet attrs) {
            ...
            // 创建一个Rect和一个Paint对象，并设置Paint对象的样式和颜色。
            mRect = new Rect();
            mPaint = new Paint();
    ...
    /**
     * 当Android首次启动Activity时调用此方法。从传入的
     * Intent，它确定需要什么类型的编辑，然后执行。
     */
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Get handles to category and todo controls
        mCategorySpinner = (Spinner) findViewById(R.id.category_spinner);
        mIsTodoCheckBox = (CheckBox) findViewById(R.id.is_todo_checkbox);
        mTodoCompletedCheckBox = (CheckBox) findViewById(R.id.todo_completed_checkbox);
        
        // Apply settings (theme, background color, text size, font)
        applySettings();
        
        // Set up todo checkbox listener
        if (mIsTodoCheckBox != null && mTodoCompletedCheckBox != null) {
            mIsTodoCheckBox.setOnCheckedChangeListener(new android.widget.CompoundButton.OnCheckedChangeListener() {
                @Override
                public void onCheckedChanged(android.widget.CompoundButton buttonView, boolean isChecked) {
                    mTodoCompletedCheckBox.setVisibility(isChecked ? CheckBox.VISIBLE : CheckBox.GONE);
                    if (!isChecked) {
                        mTodoCompletedCheckBox.setChecked(false);
                    }
                }
            });
        }
    ...
    protected void onResume() {
        ...
        if (mCursor != null) {
            ...
            // Load todo status
            if (mIsTodoCheckBox != null) {
                int colIsTodoIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_IS_TODO);
                int isTodo = mCursor.getInt(colIsTodoIndex);
                mIsTodoCheckBox.setChecked(isTodo == 1);
                
                // Show/hide completed checkbox based on is_todo
                if (mTodoCompletedCheckBox != null) {
                    mTodoCompletedCheckBox.setVisibility(isTodo == 1 ? CheckBox.VISIBLE : CheckBox.GONE);
                    if (isTodo == 1) {
                        int colTodoCompletedIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_TODO_COMPLETED);
                        int todoCompleted = mCursor.getInt(colTodoCompletedIndex);
                        mTodoCompletedCheckBox.setChecked(todoCompleted == 1);
                    }
                }
            }
...
//END_INCLUDE(paste)
    ...
    private final void updateNote(String text, String title) {
        ...
        if (mIsTodoCheckBox != null) {
            int isTodo = mIsTodoCheckBox.isChecked() ? 1 : 0;
            values.put(NotePad.Notes.COLUMN_NAME_IS_TODO, isTodo);
            
            if (mTodoCompletedCheckBox != null) {
                int todoCompleted = (isTodo == 1 && mTodoCompletedCheckBox.isChecked()) ? 1 : 0;
                values.put(NotePad.Notes.COLUMN_NAME_TODO_COMPLETED, todoCompleted);
            }
        }
    ...
    /**
     * Applies user preferences for theme, background color, text size, and font
     */
    private void applySettings() {
        SharedPreferences prefs = SettingsActivity.getPreferences(this);
        
        // Apply background color
        String bgColor = prefs.getString("background_color", "White");
        int backgroundColor = Color.WHITE;
        if (bgColor.equals("Light Gray")) {
            backgroundColor = Color.parseColor("#F5F5F5");
        } else if (bgColor.equals("Beige")) {
            backgroundColor = Color.parseColor("#F5F5DC");
        } else if (bgColor.equals("Light Blue")) {
            backgroundColor = Color.parseColor("#E3F2FD");
        } else if (bgColor.equals("Light Green")) {
            backgroundColor = Color.parseColor("#E8F5E9");
        }
        View contentView = findViewById(android.R.id.content);
        if (contentView != null) {
            contentView.setBackgroundColor(ThemeHelper.getListBackgroundColor(this));
        }
        if (mText != null) {
            mText.setBackgroundColor(backgroundColor);
            int textColor = ThemeHelper.getContrastingTextColor(backgroundColor);
            mText.setTextColor(textColor);
        }
        
        // Apply text size
        int textSize = prefs.getInt("text_size", 18);
        if (mText != null) {
